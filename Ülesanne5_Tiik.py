...
speedX1 = 2 # Muutuja speedX2 väärtus on 2.

pad = pygame.Rect(posX1, posY1, 120, 20) # luuakse rect objekt, mille asukohaks on posX2, ja posY2 ning pilt salvestatakse suurusega 120x20.
...
while (running): # alustab while tsüklit, kuniks running väärtus on tõene ehk True.
    ...
    pad = pygame.Rect(posX1, posY1, 120, 20) # Tsüklis luuakse rect objekt, mille asukohaks on posX2, ja posY2 ning pilt salvestatakse suurusega 120x20.
    screen.blit(pad1, pad) # ekraanil kuvatakse pad-i pilt mille parameetriks on muutuja pad1

    posX1 += speedX1 # muutuja posX2 väärtust suurendatakse muutuja speedX2 väärtuse võrra.

    # aluse kokkupõrke tuvastamine ning suuna muutmine.
    if posX1 > screenX - pad1.get_rect().width or posX1 < 0: #kui posX2 on suurem screenX ja pad1.get_rect().width vahest, või posX2 on väiksem kui 0
        speedX1 = -speedX1 # # muudetakse muutuja speedX2 väärtus negatiivseks, ehk asi hakkab toimuma teistpidi
